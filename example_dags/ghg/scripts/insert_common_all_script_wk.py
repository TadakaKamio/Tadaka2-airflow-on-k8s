...
@log_writer(logger)
def insert_common_mst_info_all_wk(**kwargs):
    """
    関数名：マスタデータ登録
    ...
    try:
        ...
        today_str = "20250620"  # テスト用に固定値を使用"
    ...
    try:
        ...
        if not db_is_empty:
            ...
            if not update_rows:
                ...
                logger.info("TEMP_UPDATE_TABLE にデータを挿入します。")
                ...
                logger.info("MERGE INTO による UPDATE 処理が完了しました。")
            logger.info("UPDATE 処理が完了しました。")

            delete_keys = []
            logger.info(f"DELETE処理のKEYは {delete_keys} 。")
            for row in data_rows:
                if row[1] != "9":
                    continue

                if table_name == "user_info_wk":
                    delete_keys.append((row[2],))  # 主キー：USI_USR_ID
                elif table_name == "Soshiki":
                    delete_keys.append((row[2], row[3]))  # 主キー：ORG_CD, ORG_SUB_CD
                elif table_name == "Yakusyoku":
            ...
            if delete_keys:
                if table_name == "user_info_wk":
                    logger.info("user_info_wkのDELETE 処理を開始します。")
                    ...
                    # 複合主キーは OR 結合
                    logger.info("SoshikiのDELETE 処理を開始します。")
                    conditions = [
                    ...
                    logger.info("YakusyokuのDELETE 処理を開始します。")
                    ids = "', '".join(key[0] for key in delete_keys)
                    delete_sql = (

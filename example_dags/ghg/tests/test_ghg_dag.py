import airflow
import ghg.tests.test_ghg_script as script
...
dag = DAG(
    ...
    description="GHG動作確認用DAG",  # DAGの説明
    schedule_interval="@once",  # 実行スケジュール
    tags=["ghg", run_mode],  # DAGの分類
)

# タスクの設定
task_test_ghg = PythonOperator(
    task_id="task_test_ghg",  # タスクのID
    python_callable=script.main,  # 実行するPython関数
    dag=dag,  # 所属するDAG

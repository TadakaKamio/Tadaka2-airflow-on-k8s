import airflow
import ghg.tests.li_test as script
...
dag = DAG(
    ...
    description="for test",  # DAGの説明
    schedule_interval="@once",  # 実行スケジュール
    tags=["ghg", run_mode],  # DAGの分類
)

# タスクの設定
task_li_test = PythonOperator(
    task_id="task_li_test",  # タスクのID
    python_callable=script.test,  # 実行するPython関数
    dag=dag,  # 所属するDAG

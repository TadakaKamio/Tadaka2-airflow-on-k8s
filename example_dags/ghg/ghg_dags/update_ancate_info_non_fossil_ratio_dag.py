import airflow
import ghg.scripts.update_ancate_info_script as script
...
# DAGの設定
dag = DAG(
    "update_ancate_info_non_fossil_ratio",  # DAGのID
    default_args=default_args,  # 上記で設定したデフォルトパラメータ
    description="調査表情報の非化石割合情報を更新",  # DAGの説明
    schedule_interval="@once",  # 実行スケジュール
    tags=["ghg", run_mode],  # DAGの分類
)

# タスクの設定
task_update_ancate_info_non_fossil_ratio = PythonOperator(
    task_id="task_update_ancate_info_non_fossil_ratio",  # タスクのID
    python_callable=script.update_non_fossil_ratio,  # 実行するPython関数
    dag=dag,  # 所属するDAG
    executor_config=get_k8s_config(),  # タスク実行のためのKubernetes設定を取得
)

# タスクの実行順序の設定
task_update_ancate_info_non_fossil_ratio

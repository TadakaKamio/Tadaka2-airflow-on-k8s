import airflow
import ghg.scripts.insert_shitei_info_script as script
...
# DAGの設定
dag = DAG(
    "insert_shitei_info",  # DAGのID
    default_args=default_args,  # 上記で設定したデフォルトパラメータ
    description="指定表のデータ登録",  # DAGの説明
    schedule_interval="@once",  # 実行スケジュール
    tags=["ghg", run_mode],  # DAGの分類
)

# タスクの設定
task_insert_shitei_info = PythonOperator(
    task_id="task_insert_shitei_info",  # タスクのID
    python_callable=script.insert_shitei_info,  # 実行するPython関数
    dag=dag,  # 所属するDAG
    executor_config=get_k8s_config(),  # タスク実行のためのKubernetes設定を取得
)

# タスクの実行順序の設定
task_insert_shitei_info
